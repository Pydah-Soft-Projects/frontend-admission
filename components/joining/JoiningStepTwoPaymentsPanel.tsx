...
/** Admission fee collection for Step 2 — uses Payments → Fee Configuration (per course / branch). */
        ...
        <div>
          ...
          {(courseName || branchName || quota) && (
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              ...
              {configuredFee != null ? (
                <span className="rounded-full bg-emerald-50 px-3 py-1 font-semibold text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-200">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white/90 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900/80">
            ...
            <div className="mt-4">
              ...
              {cashfreeConfig && (
                <span className="ml-2 text-[10px] uppercase text-slate-400">Cashfree: production</span>
              )}
            </div>
          </div>
          {configuredFee !== null && outstandingBalance > configuredFee && (
            <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs text-rose-600 dark:border-rose-900/60 dark:bg-rose-900/40 dark:text-rose-200">
              ...
              {transactions.map((transaction) => {
                ...
                return (
                  ...
                  >
                    ...
                    {(transaction.feeHeadName || transaction.referenceId || collectorName) && (
                      <p className="mt-1 text-xs text-slate-500">
                        {transaction.feeHeadName ? (
                          <span className="font-medium">{transaction.feeHeadName} · </span>
                        ) : null}
                        {transaction.referenceId ? (
                          <span className="font-mono">Ref: {transaction.referenceId}</span>
                        ) : null}
                        {collectorName ? <span> · {collectorName}</span> : null}
                      </p>
                    )}
                  </li>
                );
