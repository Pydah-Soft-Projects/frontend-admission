...
export function defaultJoiningPermissionExtras(): Pick<ModulePermission, 'editReference' | 'editAdmission'> {
  return { editReference: false, editAdmission: false };
}
...
export function joiningExtrasFromStored(entry?: ModulePermission): Pick<ModulePermission, 'editReference' | 'editAdmission'> {
  if (!entry?.access || entry.permission !== 'write') {
    return defaultJoiningPermissionExtras();
  }
  if (isLegacyJoiningWrite(entry)) {
    return { editReference: true, editAdmission: true };
  }
  return {
