...
const academicYearString = `${currentYear}`;
...
const UserAddLeadPage = () => {
  ...
  const headerContent = useMemo(
    () => (
      <div className="flex flex-col items-end gap-2 text-right">
        <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Add New Enquiry</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Capture a new prospect. Lead status will be automatically set to Assigned.
        </p>
      </div>
    ),
  ...
  useEffect(() => {
    ...
    setMobileTopBar({ title: 'Add Enquiry', showBack: true });
    return () => {
      clearHeaderContent();
      clearMobileTopBar();
    };
  }, [headerContent, setHeaderContent, clearHeaderContent, setMobileTopBar, clearMobileTopBar]);
  ...
  const createLeadMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      showToast.success('Enquiry created and assigned successfully');
      setFormState(initialFormState);
      setErrors({});
      const leadId = data?.data?._id || data?._id;
      if (leadId) {
        router.push(`/user/leads/${leadId}`);
      } else {
        router.push('/user/leads');
      }
    },
    onError: (error: any) => {
      showToast.error(error.response?.data?.message || 'Unable to create enquiry');
    },
  });
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4 sm:space-y-6">
      <Card className="p-4 sm:p-6 shadow-lg shadow-orange-100/40 dark:shadow-none border-0 sm:border">
        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">

          {/* Form Builder template selector - hidden for PRO as per requirements */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    {sortedFormFields.map((field: any) => {
                      ...
                      if (isLocationDropdownField(field)) {
                        ...
                        return (
                          <div key={field._id}>
                            <label className="mb-1 block text-xs sm:text-sm font-medium text-gray-700 dark:text-slate-200">
                              {field.fieldLabel}{' '}
                      ...
                      if (isSchoolOrCollegeField(field)) {
                        ...
                        return (
                          <div key={field._id}>
                            ...
                            {!hasSelectedGroup && (
                              <p className="mb-2 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">
                          ...
                          <div key={field._id} className="md:col-span-2">
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                className="h-4 w-4 rounded border-gray-300 text-orange-600 focus:ring-orange-500 focus:ring-2"
                              />
                            ...
                            {field.helpText && (
                              <p className="mt-1 text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 ml-6">
                                {field.helpText}
                              </p>
                            )}
                        ...
                        return (
                          <div key={field._id} className="md:col-span-2">
                            ...
                            <textarea
                              ...
                              required={isFieldRequired}
                              rows={4}
                              className="w-full rounded-xl border-2 border-gray-200 bg-white/80 px-3 py-2.5 sm:px-4 sm:py-3 text-xs sm:text-sm text-slate-700 shadow-sm focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-300 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
                            />
            ...
            <>
              ...
              <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
                ...
                <div>
                  ...
                  {errors.studentGroup && (
                    <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.studentGroup}</p>
                  )}
                </div>
                <div>
                  <Input
              ...
              <div className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  ...
                  {errors.district && (
                    <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.district}</p>
                  )}
                </div>
                <div>
                  <label className="mb-1 block text-xs sm:text-sm font-medium text-gray-700 dark:text-slate-200">
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border-2 border-gray-200 bg-white/80 px-3 py-2.5 sm:px-4 sm:py-3 text-xs sm:text-sm font-medium text-slate-600 transition focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-300 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100 disabled:bg-gray-100 dark:disabled:bg-slate-800 disabled:cursor-not-allowed"
                  >
                    <option value="">
                  ...
                  {errors.mandal && (
                    <p className="mt-1 text-xs sm:text-sm text-red-600">{errors.mandal}</p>
                  )}
                </div>
                <div>
                  <label className="mb-1 block text-xs sm:text-sm font-medium text-gray-700 dark:text-slate-200">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-3">
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              <Button
                type="button"
                variant="outline"
                className="flex-1 sm:flex-initial text-xs sm:text-sm h-10 sm:h-11"
                onClick={() => {
                  setFormState(initialFormState);
                  setErrors({});
                }}
              ...
              <Button 
                ...
                disabled={createLeadMutation.isPending}
              >
                {createLeadMutation.isPending ? 'Saving…' : 'Create Enquiry'}
              </Button>
            </div>
          </div>
...
export default UserAddLeadPage;
